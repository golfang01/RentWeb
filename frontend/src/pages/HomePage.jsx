...
const MOCK_HOT = [
  { product_id: 'm1', product_name: 'Sony A7 IV Body', description: 'Full-frame mirrorless camera', daily_rate: 1200, deposit_amount: 5000, availability_status: 'available', images: [], shop_name: 'Camera Pro Shop', rating_score: 4.9, rent_count: 128 },
  { product_id: 'm2', product_name: 'DJI Mini 3 Pro', description: 'โดรนขนาดเล็ก บินง่าย ภาพสวย', daily_rate: 900, deposit_amount: 4000, availability_status: 'available', images: [], shop_name: 'Sky Rental', rating_score: 4.8, rent_count: 95 },
  { product_id: 'm3', product_name: 'GoPro Hero 11', description: 'Action camera ความละเอียดสูง', daily_rate: 450, deposit_amount: 2000, availability_status: 'available', images: [], shop_name: 'Adventure Gear', rating_score: 4.7, rent_count: 210 },
  { product_id: 'm4', product_name: 'Canon EOS R5', description: 'Canon R5 พร้อม lens 24-70mm', daily_rate: 1800, deposit_amount: 8000, availability_status: 'available', images: [], shop_name: 'Lens World', rating_score: 5.0, rent_count: 76 },
];

const MOCK_RECOMMENDED = [
  { product_id: 'm5', product_name: 'Tent Coleman 4 คน', description: 'เต็นท์แคมป์ปิ้ง กันน้ำ กันลม', daily_rate: 350, deposit_amount: 1500, availability_status: 'available', images: [], shop_name: 'Camp Gear', rating_score: 4.6, rent_count: 55 },
  { product_id: 'm6', product_name: 'MacBook Pro M3', description: 'Laptop สำหรับงาน Creative', daily_rate: 800, deposit_amount: 10000, availability_status: 'available', images: [], shop_name: 'Tech Rental', rating_score: 4.8, rent_count: 43 },
  { product_id: 'm7', product_name: 'Guitar Fender Stratocaster', description: 'กีตาร์ไฟฟ้า Fender Classic', daily_rate: 300, deposit_amount: 3000, availability_status: 'available', images: [], shop_name: 'Music Store', rating_score: 4.5, rent_count: 31 },
  { product_id: 'm8', product_name: 'Projector Epson 4K', description: 'โปรเจคเตอร์ 4K ความสว่างสูง', daily_rate: 600, deposit_amount: 5000, availability_status: 'rented', images: [], shop_name: 'AV Rental', rating_score: 4.7, rent_count: 88 },
];
...
const getProductEmoji = (name = '') => {
  ...
  if (n.includes('camera') || n.includes('canon') || n.includes('sony') || n.includes('nikon')) return '📷';
  ...
  if (n.includes('bike') || n.includes('จักรยาน')) return '🚴';
  return '📦';
};
      ...
      <div style={{
        ...
        <div style={{ padding: '20px 24px', borderBottom: '1px solid #f3f4f6', display: 'flex', gap: '14px', alignItems: 'center' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '16px', fontWeight: '800', color: '#f97316', marginTop: '4px' }}>
              ฿{Number(product.daily_rate).toLocaleString()}<span style={{ fontSize: '12px', color: '#9ca3af', fontWeight: '400' }}>/วัน</span>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ padding: '20px 24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
            {[
              { label: '📅 วันที่รับสินค้า', value: startDate, min: today, onChange: setStartDate },
              { label: '📅 วันที่คืนสินค้า', value: endDate, min: startDate, onChange: setEndDate },
            ].map((f) => (
              <div key={f.label}>
          ...
          <div style={{ backgroundColor: '#f9fafb', borderRadius: '14px', padding: '16px', marginBottom: '16px' }}>
            ...
            <div style={{
              display: 'flex', justifyContent: 'space-between', alignItems: 'center',
              borderTop: '2px solid #e5e7eb', marginTop: '10px', paddingTop: '10px',
            }}>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            ...
            <button type="submit" disabled={loading} style={{
              ...
              {loading ? 'กำลังส่งคำขอ...' : '✅ ยืนยันการเช่า'}
            </button>
          </div>
        </form>
      </div>
...
const SuccessToast = ({ onClose }) => {
  ...
  return (
    <div style={{
      ...
      <div>
        <div style={{ fontWeight: '700', fontSize: '15px' }}>ส่งคำขอเช่าสำเร็จ!</div>
        <div style={{ fontSize: '12px', color: '#9ca3af', marginTop: '2px' }}>รอเจ้าของร้านอนุมัติใน "การจองของฉัน"</div>
      </div>
    </div>
  );
};
...
const ProductRow = ({ products, onRent, showRank }) => (
  <div style={{
    display: 'grid',
...
const HomePage = () => {
  ...
  const fetchCategories = useCallback(async () => {
    ...
    try {
      ...
      // โหลด Hot + Recommended จาก 2 categories แรก (ถ้ามี)
      if (cats.length >= 1) {
        loadDefaultProducts(cats);
      } else {
  ...
  const loadDefaultProducts = async (cats) => {
    setLoadingDefault(true);
    try {
      // ดึงสินค้าจาก category แรก → Hot Deal
      const r1 = await categoryService.getProductsByCategory(cats[0].category_id);
      const p1 = r1.data.data || [];
      setHotProducts(p1.length > 0 ? p1.slice(0, 4) : MOCK_HOT);

      // ดึงสินค้าจาก category ที่ 2 → Recommended (ถ้ามี)
      if (cats.length >= 2) {
        const r2 = await categoryService.getProductsByCategory(cats[1].category_id);
        const p2 = r2.data.data || [];
  ...
  // โหลด products ตาม category ที่เลือก
  ...
  const handleBookingSuccess = () => { setBookingProduct(null); setShowSuccess(true); };

  // Skeleton card
  const SkeletonCard = () => (
    <div style={{ backgroundColor: '#f3f4f6', borderRadius: '18px', height: '260px', animation: 'pulse 1.5s infinite' }} />
  );
  ...
  return (
    <div style={{ minHeight: '100vh', width: '100%', backgroundColor: '#FFF8F3' }}>
      ...
      <div style={{
        ...
        <h1 style={{ color: '#fff', fontSize: '34px', fontWeight: '800', marginBottom: '8px', letterSpacing: '-0.5px' }}>
          เช่าสิ่งที่คุณต้องการ 🚀
        </h1>
        <p style={{ color: '#fed7aa', fontSize: '16px', marginBottom: '30px' }}>
          กล้อง โดรน อุปกรณ์แคมป์ปิ้ง และอีกมากมาย — เช่าง่าย คืนสะดวก
        </p>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '36px 24px 60px' }}>
        ...
        {/* ===== ยังไม่เลือก category → แสดง Default Sections ===== */}
        {!selectedCategory ? (
          <>
            {/* Hot Deals */}
            <div style={{ marginBottom: '48px' }}>
              <SectionHeader
                emoji="🔥"
                title="Hot Deals"
                subtitle="สินค้ายอดนิยม เช่าเยอะที่สุดในขณะนี้"
              />
              {loadingDefault ? (
                ...
                <ProductRow products={hotProducts} onRent={setBookingProduct} showRank />
              )}
            </div>
            ...
            <div style={{ marginBottom: '48px' }}>
              <SectionHeader
                ...
                subtitle="สินค้าแนะนำที่คัดมาเพื่อคุณ"
              />
              {loadingDefault ? (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(215px, 1fr))', gap: '16px' }}>
                  {[...Array(4)].map((_, i) => <SkeletonCard key={i} />)}
                ...
                <ProductRow products={recommendedProducts} onRent={setBookingProduct} />
              )}
            </div>
            ...
            <div style={{
              ...
              <div>
                ...
                <p style={{ color: '#9ca3af', fontSize: '14px', margin: 0 }}>
                  เปิดร้านและเริ่มสร้างรายได้วันนี้ — ฟรี ไม่มีค่าธรรมเนียม
                </p>
              </div>
          ...
          <>
            ...
            {loadingProducts ? (
              ...
              <ProductRow products={filteredProducts} onRent={setBookingProduct} />
            )}
          </>
        )}
      </div>
      ...
      {bookingProduct && (
        <BookingModal product={bookingProduct} onClose={() => setBookingProduct(null)} onSuccess={handleBookingSuccess} />
      )}
