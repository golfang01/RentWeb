...
class PaymentController {
  ...
  async uploadSlip(req, res) {
    try {
      ...
      if (!req.file) {
        return res.status(400).json({ success: false, message: 'กรุณาแนบรูปสลิปภาพไฟล์' });
      }
      const slip_image_url = `slips/${req.file.filename}`; // ได้ชื่อแบบ 1710039200000.jpg

      // 1. ดึง booking (และเช็ค cancelled)
      ...
      // ✅ ถ้าถูกยกเลิกแล้ว ห้ามอัปโหลด
      if (booking.status === 'cancelled') {
        return res.status(400).json({
          success: false,
          message: `รายการนี้ถูกยกเลิกแล้ว ไม่สามารถอัปโหลดสลิปได้`,
        });
      }
      // อนุโลมเฉพาะ approved/confirmed
      ...
      // อัพเดท booking ถ้าสถานะเดิม “ไม่ใช่ cancelled” เท่านั้น
      await pool.query(
        `UPDATE bookings SET status = 'waiting_verification' WHERE booking_id = $1 AND status != 'cancelled'`,
        [booking_id]
      );
  ...
  async rejectPayment(req, res) {
    try {
      ...
      await pool.query(
        `UPDATE Payments SET payment_status = 'rejected', reject_reason = $2, updated_at = NOW() WHERE booking_id = $1`,
        [booking_id, reason || null]
      );
      // คืนสถานะเป็น confirmed เพื่อให้ user อัปโหลดสลิปใหม่
  ...
  SELECT pay.*, b.status AS booking_status, b.total_price, b.deposit_held, p.title AS product_name, u.full_name AS payer_name
FROM Payments pay
